      ...
      <div class="popAbout" v-if="pop">
        <ul>
          <li
            ...
            @click="resumeFind(item.resume_id,item.name,item.push_status,item.recommend_status)"
          >{{item.name}}</li>
        </ul>
      </div>
        ...
        <div class="RecruitmentInformation" v-for="items in resumement" :key="items.index">
          <div @click="toDetail(items.push_resume_id),readStatus(items.push_resume_id)">
            <div class="establish_show">
              <span v-if="items.t1_name!=null" class="add_eslit">
                {{items.t1_name}}-{{items.t2_name}}
                <b
                  v-if="items.read_status == 0"
                  class="roundRed"
                >●</b>
              </span>
              <span v-else-if="items.t1_name==null" class="add_eslit">
                {{items.t2_name}}
                <b v-if="items.read_status == 0" class="roundRed"></b>
              </span>
    ...
    toDetail(resumeId) {
      ...
      if (user_type == null || user_type == 1) {
        this.$router.push({
          ...
          query: { reusemId: resumeId,globalResumeid:this.globalResumeid }
        });
      } else if (user_type == 2) {
        this.$router.push({
